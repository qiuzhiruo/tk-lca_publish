...
def apply(xml_path,pass_name='default'):
    ...
    # Keep the shader assignment info
    shading_info = {}
    shading_grps = cmds.ls(type = 'shadingEngine')
    if 'initialParticleSE' in shading_grps:
        shading_grps.remove('initialParticleSE')
    if 'initialShadingGroup' in shading_grps:
        shading_grps.remove('initialShadingGroup')

    for i in shading_grps:
        shading_info[i] = cmds.sets(i, q=True)
    ...
    for ind, sh in enumerate(color_shader_list):
        if cmds.objExists(sh):
            color_shader_list[ind] = cmds.rename(sh, 'surface_' + ver_path.split('.')[-1] + '_rgbColor1')
    
    for ind, sh in enumerate(map_shader_list):
        if cmds.objExists(sh):
            map_shader_list[ind] = cmds.rename(sh, 'surface_' + ver_path.split('.')[-1] + '_textureMap1')

    return shading_info, color_shader_list + map_shader_list
