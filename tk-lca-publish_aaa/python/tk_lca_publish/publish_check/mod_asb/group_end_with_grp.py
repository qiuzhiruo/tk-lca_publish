...
class StdCheck():
    ...
    def __init__(self, dialog):
        self.dialog = dialog
        self.check_name = u"asb内的组名称必须以_grp结尾"
        self.description = u"asb内如果有自定义的组，则组名称必须以_grp结尾"
        self.auto_fix = False
        self.duty = u"艺术家本人。"
        return

    def getTransform(self, top='|master|asb'):
        l_AR_keys = ['|' + n.nodeName() + '|' for n in pm.listRelatives(top, type='assemblyReference', ad=True)]
        trans = []
        for c in pm.listRelatives(top, ad=True):
            ...
            ar_node = False
            for key in l_AR_keys:
                if key in c.fullPath():
                    ar_node = True
            if not ar_node:
                trans.append(c.name())
        return trans
