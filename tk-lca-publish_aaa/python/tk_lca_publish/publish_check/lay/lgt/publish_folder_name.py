...
class StdCheck():
    ...
    def run_check(self):
        ...
        if not re.match(str(self.dialog.entity['name'])+'\.lgt\.comp\.v\d{3}.nk$', os.path.basename(self.dialog.nk_file)):
        ...
        if 'R1' in self.dialog.version_tag:
            ...
            resolution_tag='R3'
        if ('/'+resolution_tag+'/') not in l_folder or ('/'+resolution_tag+'/') not in r_folder:
        ...
        if version_name_check != r_version_name:
            return u'L R版本号不匹配'
