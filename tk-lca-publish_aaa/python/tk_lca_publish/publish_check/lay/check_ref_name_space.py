        ...
        try:
            ...
            if wrong_ns_ref:
                return u'以下资产名称空间不一致: \n' + '\n'.join(
                    [pm.referenceQuery(ref_node, nodes=True, dagPath=1)[0] for ref_node in wrong_ns_ref])
            else:
                return ''
        except:
    ...
    def findWrongNameSpace(self):
        ...
        for r in refs:
            try:
                ...
                if '/cam/publish/' in file_ref.path:
                    continue

                # ref_file_path = file_ref.path.split('{')[0]
                # if '/publish/' in ref_file_path and '/cache/' in ref_file_path and ref_file_path.endswith('.abc'):
                #     continue
                ...
                if pm.referenceQuery(file_ref, rfn=True, parent=True):
                    ...
                    if perfect_ns != ns_src:
                        ...
                        print "命名空间携带空的父级命名空间, 应该手动修正: " + str(file_ref.refNode)
                        continue
                ...
                if not ns_src or ns_src == ':':
                    fake_ns.append(file_ref.refNode)
                    print "该资产没有命名空间: " + str(file_ref.refNode)
                    continue

                file_path = str(file_ref.path).strip()
                if ns_src != os.path.basename(file_path)[:-3]:
                    fake_ns.append(file_ref.refNode)
                    print '命名空间与资产名不匹配: ' + str(file_ref.refNode)
            except:
                print traceback.format_exc()
                print "命名空间异常!: " + str(file_ref.refNode)
                fake_ns.append(file_ref.refNode)
