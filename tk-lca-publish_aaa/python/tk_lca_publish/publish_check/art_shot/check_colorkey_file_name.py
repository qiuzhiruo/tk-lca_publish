...
class StdCheck():
    ...
    def __init__(self, dialog):
        self.dialog = dialog
        self.check_name = u"检查提交的预览文件的命名。"
        self.description = u"提交的预览文件名由a-z的字母，0-9数字，下划线\"_\"和点\".\"组成，英文字母全小写。\n文件路径所在的各级文件夹命名不能有中文和空格。"
        self.auto_fix = False
        self.duty = u"艺术家本人。"
        return

    def check_shot_exist(self, shot_name):
        shot_data = self.dialog.sg.find_one('Shot', [['project', 'name_is', self.dialog.project['name'].lower()], ['code', 'is', shot_name]], ['id'])
        return shot_data
    ...
    def run_check(self):
        try:
            ...
            for file_path_qtstr in l_preview_files:
                ...
                self.dialog.print_log('pub image path :' + file_path)

                file_name = file_path.split('/')[-1]
                f = file_name.split('.')[0]
                shot_code.append(f)
                if not re.match(p3, f):
                    return u"文件命名不规范，提交的预览文件名请以镜头号开始命名:\n" + "\n".join(shot_code)
            ...
            for s in shot_code:
                if not self.check_shot_exist(s):
                    return proj + u"项目中没有此镜头号:\n" + s
