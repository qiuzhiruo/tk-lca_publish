...
class StdCheck():
    ...
    def __init__(self, dialog):
        self.dialog = dialog
        self.check_name = u"检查plt资产内节点和组命名"
        self.description = u"在master下有plt组。Plt组内毛发节点命名为<描述>_<类别>。没有"
        self.auto_fix = True
        self.duty = u"艺术家本人"
        return


    def node_cnt(self, node_name):
    ...
    def run_check(self):
        try:
            ...
            if task_name.startswith('plt'):
                if not pm.objExists("|master|plt"):
                    return u"没有找到最高层master下面的plt组。"

                hair_node = pm.PyNode('|master|plt')
                for chd in hair_node.getChildren():
                    chd_name = chd.name()
                    if nameIsHairSystem(chd_name) and len(chd_name.split('_')) != 2:
                        return chd_name+u'不符合plt命名规范,请参考downL_pfxHair'

            for node_name in ['poly','hi', 'lo', 'rig','shape']:
                if self.node_cnt(node_name) > 1:
                    return u'在master组下发现重名节点: ' + node_name
