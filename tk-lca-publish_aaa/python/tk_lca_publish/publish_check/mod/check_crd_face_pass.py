...
class StdCheck():
    def __init__(self, dialog):
        self.dialog = dialog
        self.check_name = u"检查crd资产模型衣服与头发pass层级结构与命名规范。"
        self.description = u"规范验证 cloth_pass_grp 和 hair_pass_grp 下的组命名。仅验证二级组名及必须包含的默认组，不对内部Mesh进行校验。"
        self.auto_fix = False
        self.duty = u"艺术家本人。"
        return
    ...
    def run_check(self):
        ...
        try:
            ...
            if self.dialog.project['name'].lower() in ['lic']:
                return ''
            if asset_info.get('sg_asset_type') != 'crd':
                return ''
            # crd_变脸资产，制片必须标注 ‘crd_face_pass’的tags
            if not tag_list or 'crd_face_pass' not in tag_list:
                return ''
            ...
            hair_pass_grp = '|master|poly|hi|mesh_grp|hair_grp|head_hair'
            suffix = "_grp"
            errors = []
            if not cmds.objExists(cloth_pass_grp):
                return ''
            if not cmds.objExists(hair_pass_grp):
                return '当前crd资产制片标注了tag:crd_face_pass，是多pass 的群集资产，\n请确保|master|poly|hi|mesh_grp|hair_grp|head_hair组存在'
            # 各通道特定的前缀配置
            pipe_configs = {
                cloth_pass_grp: {
                    "prefix": "CLT_",
            ...
            for fc in face_children:
                if (not fc.endswith("_grp")) or (not fc.startswith("face_")):
                    return ''
