...
class StdCheck():
    ...
    def __init__(self, dialog):
        self.dialog = dialog
        self.check_name = u"一个点相连的边数不能大于相连的面多于1。"
        self.description = u"一个点相连的边数不能大于相连的面多于1。一般情况下一个顶点最多连四个面，也就是四条棱，最多不超过5个面。"
        self.auto_fix = False
        self.duty = u"艺术家本人。"
        return
    ...
    def run_check(self):
        try:
            ...
            l_meshes = cmds.listRelatives('|master|poly', ad=True, type='mesh', path=True)
            ...
            for mesh_node in l_meshes:
                ...
                face_cnt = om.MIntArray()
                while not itMesh.isDone():
                    itMesh.getConnectedEdges(edge_cnt)
                    itMesh.getConnectedFaces(face_cnt)
                    if len(edge_cnt)-len(face_cnt) > 1:
                        print 'face_cnt : ',len(edge_cnt),len(face_cnt)
            ...
            if len(l_invalid_meshes) > 0:
                ...
                return u'有些polygon几何体的顶点相连的边数比相连的面多于1:\n'+', '.join(l_invalid_meshes)
