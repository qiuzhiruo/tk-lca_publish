    ...
    def run_check(self):
        ...
        try:
            ...
            for asset_name in self.dialog.d_assets_info.keys():
                ...
                if asset_type in ['chr']:
                    sg_info = self.get_asset_shotgun_info(asset_name=asset_name)
                    if 'special_hierarchy_chr' in sg_info['tag_list']:
                        print "special_hierarchy_chr skip"
                        return ''
                if asset_type != 'flg':
                    check_hier = ['mesh_grp', 'hi', 'poly', 'master']
                    error_h = []
                    proxy = False
                    for h in check_hier:
                        if len(pm.ls(h)) < 1:
                            self.auto = False
                            return '缺少{}组.'.format(h)
                        ...
                        if h_c > 1:
                            self.auto = False
                            return '出现了多个{}组.'.format(h)

                        if h == 'mesh_grp' and len(pm.ls('*'+h+'*')) > 1 and h_c > 1:
                            error_list = [m.name() for m in pm.ls('*'+h+'*') if m.name() != h and m in pm.listRelatives('master', ad=True, type='transform') and m.name() != 'mesh_grp_lo']
                            if error_list:
                                self.auto = False
                                return '这些层级,名字里含有mesh_grp, 请修改掉：\n{}'.format('\n'.join(error_list))

                        for i in pm.ls(h+'*'):
                            if i.name().replace(h, '').isdigit():
                                error_h.append(i.name())

                    if error_h:
                        self.auto = False
                        return '这些层级与 hi 或者 poly 或者 master 重名 请修改掉：\n{}'.format('\n'.join(error_h))
