...
class StdProcess():
    ...
    def create_version_preview(self, is_aces=False):
        ...
        if '.mov' in [n[-4:] for n in self.dialog.l_preview_files]:
            ...
            if src_seq == '' and 'R3' in self.dialog.version_tag and self.dialog.publish_mode == 0:
                ...
                for i in range(len(self.dialog.l_preview_files)):
                    ...
                    dst = preview_dir + '/tmp_exr/' + (self.dialog.version_name + ('.%04d.' % i) + 'exr')
                ...
                src_seq = preview_dir + '/tmp_exr/' + self.dialog.version_name + '.%04d.exr'

            if src_seq == '':
                # 正常灯光publish是不会需要用到这个copy素材的, 如果src_seq为空, 需要检查是什么原因导致的!
                # 20260916 新增: 如果是R3单帧, 那么get_src_sequence检查不通过, 只能走copy
                raise u"素材缺失, please call TD! 需要检查是什么导致的！"
