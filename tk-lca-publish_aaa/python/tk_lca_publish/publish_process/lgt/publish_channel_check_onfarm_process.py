...
def create_channel_check_task(proj, shot):
    info = sg.find('Task',
                   [['project', 'name_is', proj.upper()], ['entity', 'name_is', shot], ['step', 'name_is', 'lgt']],
...
def create_channel_check_dir_on_server(proj, shot):
    ...
    seq = shot[:3]
    target_preview_file_path = "/mnt/proj/projects/{proj}/shot/{seq}/{shot}/lgt/publish/{shot}.lgt.channel_check.{version}/preview".format(
        proj=proj, seq=seq, shot=shot, version=version)

    target_jpg_file_path = "/mnt/proj/projects/{proj}/shot/{seq}/{shot}/lgt/publish/{shot}.lgt.channel_check.{version}/jpg".format(
        proj=proj, seq=seq, shot=shot, version=version)
...
def generate_mov(proj, shot, version, target_preview_file_path):
    ...
    #     proj=proj, seq=seq, shot=shot, version=version)
    target_lighting_exr_path = "/mnt/proj/projects/{proj}/shot/{seq}/{shot}/lgt/publish/{shot}.lgt.lighting.{version}/exr/L".format(
        proj=proj, seq=seq, shot=shot, version=version)

    target_link_lighting_exr_path=os.readlink(target_lighting_exr_path)+"/{shot}.lgt.comp.####.exr".format(shot=shot)
    ...
    target_preview_mov_path = target_preview_file_path + "/{shot}.lgt.channel_check.{version}.mov".format(shot=shot,
                                                                                                          version=version)
    target_preview_jpg_path = target_nuke_proj_path + "/jpg/{shot}.lgt.channel_check.####.jpg".format(shot=shot,
                                                                                                          version=version)
    ...
    cmd="{rvio_path} {source_jpg} -pa 1.0  -quality 1 -outrgb -o {target_preview_mov_path}".format(rvio_path=rvio_path,
...
def delete_sg_repeat_version(proj, shot, version):
    task_info_dict = sg.find_one("Task",
                                 [
                                     ['project', 'name_is', proj], ['entity', 'name_is', shot],
                                     ['content', 'is', 'channel_check'], ['step', 'name_is', 'lgt']
                                 ])
    version_info_list = sg.find('Version',
                                [
                                    ['project', 'name_is', proj], ['entity', 'name_is', shot],
    ...
    target_version_code = "{shot}.lgt.channel_check.lighting.{version}".format(shot=shot, version=version)
    delete_code_list = []
...
def create_sg_version(proj, shot, version, target_preview_mov_path):
    ...
    description = "代码自动生成，对应lighting publish {version}版本".format(version=version)
    ...
    target_code = "{shot}.lgt.channel_check.{version}".format(shot=shot, version=version)
