...
class StdProcess():
    ...
    def __init__(self, dialog):
        self.dialog = dialog
        self.process_name = u"mb文件处理"
        self.description = u"mb文件材质处理。"
        return
    ...
    def proceed(self):
        if cmds.referenceQuery('|master', isNodeReferenced=True):
            ...
            return ''
        return '' # push texture already add to apply_latest_uv
        task_name = self.dialog.task['name'].lower()
        # if "rigging" in task_name:
        chr_name = self.dialog.entity['name']
        self.dialog.anim_file = self.dialog.version_dir + '/anim_rig/'
        publish_rig_mb_file = self.dialog.anim_file + chr_name + '.mb'

        path = cmds.file(q=True, sn=True)
        chr_name = path.split("/rig")[0].split("/")[-1]
        proj = path.split("projects/")[-1].split("/asset")[0]
        asset_list = []
        asset_list.append(chr_name)
        print '\n-----------push shader-----------------\n'
        import srf.push_shader.push_shader as ps
        reload(ps)
        out_cmds = ps.push_assets_shader_to(proj, asset_list, step='rig', push_shader=True)
        for c in out_cmds:
            try:
                print '\n--------------cmd'
                print c
        ...
        # cmds.file(publish_rig_mb_file, open=True, force=True)
        #
        # try:
        #     cmds.lockNode('initialShadingGroup', lock=False, lockUnpublished=False)
        #     cmds.lockNode('renderPartition', lock=False, lockUnpublished=False)
        ...
        #     #                     message='chr_name: {}, \n file_path: {} \n srf file not exist! \n {}'.format(chr_name, publish_rig_mb_file, srf_publish_dir),
        #     #                     app='warning')
        #     #     return ""
        #     #
        #     # latest_uv = ''
        ...
        #     #     lca_feishu.main(to_user_list=['xiaoyu2', 'zejie', 'zhenlin', 'pangxuan', 'wangqi2'],
        #     #                     title=u'apply_uv UV拉取失败 !',
        #     #                     message='chr_name: {}, \n file_path: {} \n uv file not exist! \n {}'.format(chr_name, publish_rig_mb_file, uv_file),
        #     #                     app='warning')
        #
        # except:
        #     from production.feishu_utils import lca_feishu
        #     lca_feishu.main(to_user_list=['xiaoyu2', 'zejie', 'zhenlin', 'pangxuan', 'wangqi2'],
        #                     title=u'apply_uv UV拉取失败 !',
        #                     message='chr_name: {}, \n file_path: {}'.format(chr_name, publish_rig_mb_file),
        #                     app='warning')
        #
        # try:
...
# Z:projects/xun/asset/chr/runtu_child_test_c/srf/publish/runtu_child_test_c.srf.surfacing/scene_graph_xml/runtu_child_test_c.uv
