...
class StdProcess():
    ...
    def delete_chr(self,c_asset_names,c_asset_name):
        ...
        old_list = mc.sets(c_asset_name+"_set", q=1)
        del_list =[]
        for a in c_asset_names:
            if c_asset_name not in a:
               other_list = mc.sets(a+"_set", q=1)
               for b in other_list:
    ...
    def proceed(self):
        ...
        if ast_info['sg_asset_type'] == 'asm':
            ...
            Project_info = self.dialog.sg.find_one('Project', [['name', 'is', self.dialog.project['name']]])

            for asset in c_asset_names:
                asset_entity = self.dialog.sg.find_one('Asset', [['project', 'is', Project_info], ['code', 'is', asset]])
                status = self.dialog.sg.find_one('Asset', [['id', 'is', asset_entity['id']]], ['sg_status_list'])
                if status['sg_status_list'] == 'omt':
                    c_asset_names.remove(asset)
            ...
            for c_asset_name in get_char_name:
                print "c_asset_name",c_asset_name
            ...
            # for c_asset_name in c_asset_names:
                if c_asset_name in c_asset_names:
                    ...
                    # 2.get asset info
                    c_asset_entity = self.dialog.sg.find_one('Asset',[['project', 'is', c_proj_info], ['code', 'is', c_asset_name]])
                    ...
                    self.dialog.d_assets_info[self.dialog.entity['name']]['node']=pm.PyNode('|master')
                    print "chr   ",c_asset_names, c_asset_name, get_char_name
                    ...
                    try:
                        import webbrowser
                        url = "http://shotgun.zhuiguang.com/detail/Asset/" + str(self.dialog.entity['id']) + "#Task_" + str(c_task['id'])+"_"+str(c_task_name)
    ...
    def processes(self, c_asset_names=None, c_asset_name=None, get_char_name=None):
        ...
        # tank_file = r"D:/rigwork/" + self.dialog.entity['name'] + '.ma'
