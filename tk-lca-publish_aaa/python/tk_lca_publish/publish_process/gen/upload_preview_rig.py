...
class StdProcess():
    ...
    def __init__(self, dialog):
        self.dialog = dialog
        self.process_name = u"上传缩略图(RIG专用)"
    ...
    def create_version_preview(self):
        ...
        if '.mov' in [n[-4:] for n in self.dialog.l_preview_files]:
            ...
            self.dialog.print_log('edit slate output for master :')

            if self.dialog.step['name'] == 'lgt' or self.dialog.step['name'] == 'pfx':
    ...
    def rig_preview(self):
        ...
        只对rig环节提交有效果
        """
        if not "rig.rigging" in str(self.dialog.v_preview):
            return

        camera_info = self.camera_info()
        mc.modelEditor(camera_info["persp"], e=1, nurbsCurves=False)
    ...
    def playblast(self):
        ...
        #     mc.refresh()
        # mc.refresh()
        # pointNode = mc.pointConstraint("spine_M_root_ctrl", "persp", mo=True)

        # persp_shape = mc.listRelatives("persp", s=True)[0]
        # mc.setAttr("{}.panZoomEnabled".format(persp_shape), 1)
        # mc.setAttr("{}.zoom".format(persp_shape), 0.75)

        mov_filename = "{}preview/publish.mov".format(self.dialog.v_preview.split("preview/")[0])
        mc.playblast(fp=4, clearCache=1, format='qt', sequenceTime=0, showOrnaments=1, percent=100,
                     filename=mov_filename, viewer=0, forceOverwrite=1, quality=100, widthHeight=(1920, 1080),
                     compression="PNG")

        # mc.setAttr("{}.panZoomEnabled".format(persp_shape), 0)
        # mc.setAttr("{}.zoom".format(persp_shape), 1)
    ...
    def camera_info(self):
        ...
        camera_info = {}
        for i in allPanel:
            mc.modelEditor(i, e=1, nurbsCurves=False)
            camreaName = mc.modelEditor(i, query=1, camera=1)
            camera_info[camreaName] = i
        return camera_info
