    ...
    def proceed(self):
        try:
            ...
            if self.dialog.step['id'] == 10 and self.dialog.task['name'] == 'model':
                if self.has_aaa_mark() and self.dialog.version_tag == u"精模":
                ...
                if self.dialog.task['name'].endswith('hair'):
                    if self.has_aaa_mark():
                        self.do_approve(reason=u"hair 任务之前有艺术通过的版本，现在新 PA 了 donwstream 版本")
                elif self.dialog.task['name'].endswith('cloth'):
                    self.do_approve()
                ...
                if  self.dialog.task['name'] == 'animation':
                    ...
                    # z1 场次,任务状态为 rtk 时,pub ds 直接设置为 sc
                    if 'z1' in self.dialog.entity['name']:
                        task_info = self.dialog.sg.find_one('Task', [['id', 'is', self.dialog.task['id']]], ['sg_status_list'])
                        if task_info['sg_status_list'] in ['rtk']:
                            self.do_approve(reason=u"z1 场次 animation 任务 为 rtk，现在新 PA 了 donwstream 版本,修改状态为 sc")
