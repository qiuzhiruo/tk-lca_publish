...
class TagsInFrustum():
    ...
    def filterObjects(self, obj):
        ...
        for o in obj:
            if pm.referenceQuery(o, inr=True):
                ...
                if pm.nodeType(o) == 'assemblyReference':
                    # TODO do we need to filter the asb and scn AR nodes?
                    masters.append(o)
                else:
                    tokens = o.split(':')
        ...
        print("{:-<30}".format('tagsInFrustum.py'))
        # print(masters)
        print("{:-<30}".format('tagsInFrustum.py'))
    ...
    def getObjectInFrustum(self, autoGetCamera=True, width=2048, height=858, mode=1, offset=1, invert=0, start=1001, end=1100):
        ...
        cam_org_info = {}
        for c in cameras:
            cam_org_info[c] = {'overscan': c.attr('overscan').get(),
                               'nearClipPlane': c.attr('nearClipPlane').get()}
            try:
                c.attr('overscan').set( overscan )
                c.attr('nearClipPlane').set( nearClipPlane )
            except:
        ...
        for c in cameras:
            try:
                c.attr('overscan').set( cam_org_info[c]['overscan'] )
                c.attr('nearClipPlane').set( cam_org_info[c]['nearClipPlane'] )
            except:
                print 'Camera', c.name(), 'is locked. Skip.'
