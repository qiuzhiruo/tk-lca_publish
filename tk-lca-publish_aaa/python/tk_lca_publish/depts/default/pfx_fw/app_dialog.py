...
class AppDialog(PublishDialog):
    ...
    def get_input_version(nk_file):
        f = open(nk_file, 'r')
        content = f.read()
        f.close()
        parts = content.split('Read ')
        sections = []
        for i in range(1 ,len(parts)):
            if len(re.findall('{', parts[i]))==1 and len(re.findall('}', parts[i]))==1:
                # isolatied Read node, ignore
                continue
            else:
        ...
        pfx_versions = []
        for l in sections:
            pfx_versions.extend(['pfx version: v'+j for j in re.findall('/pfx/output/v(\d{3})/', l)])
            pfx_versions.extend(['pfx version: v'+j for j in re.findall('/pfx/publish/[a-z]\d{5}\.pfx\.paint_fix\.v(\d{3})/', l)])
            lgt_versions.extend(['lgt version: v'+j for j in re.findall('/lgt/output/nuke/R1/v(\d{3})/', l)])
            lgt_versions.extend(['lgt version: v'+j for j in re.findall('/lgt/publish/[a-z]\d{5}\.lgt\.lighting\.v(\d{3})/', l)])
        
        pfx_versions = sorted(list(set(pfx_versions)))
        lgt_versions = sorted(list(set(lgt_versions)))

        result = ''
        if pfx_versions:
            result += pfx_versions[-1] + ' '
        if lgt_versions:
            result += lgt_versions[-1] + ' '
        return result
    ...
    def on_pick_l(self):
        ...
        # render dir should be like /output/projects/tpr/shot/seq/shot/pfx/output/floating_window/v002/L
        ...
        if os.path.isdir(render_dir):
            ...
            try:
                # work_shot: /mnt/work/projects/tpr/shot/seq/shot/pfx
                work_shot=render_dir.replace('/output/projects/', '/mnt/work/projects/').replace('/output/floating_window/', '/task/nuke/').rsplit('/',2)[0]
                version_num=render_dir.split('/')[-2]
                nuke_file=work_shot+'/'+self.entity['name']+'.'+self.step['name']+'.floating_window.'+version_num+'.nk'
    ...
    def on_pick_nk(self):
        ...
        pick_dialog.setNameFilters(["Pfx Config File (*.nk)"])
        ...
        if os.path.isfile(render_dir):
            ...
            lgt_version=self.get_input_version(nk_file)
            if lgt_version:
