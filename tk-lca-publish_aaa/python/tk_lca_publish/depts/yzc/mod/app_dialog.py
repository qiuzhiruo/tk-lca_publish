...
class AppDialog(PublishDialog):
    ...
    def get_omit_check(self,check):
        if self.step['name'] in ['mod'] and self.w_sys.comboBox_tag.currentIndex() == 0:
            ...
            omit_mod_list=['check_hair_patch','dynamic_file_name','lod_match','one_shaer_per','extremely_short_edge','uv_holes','exceed_range_uv']
            if check.module_name in omit_mod_list and mod_type in ['env','prp']:
                return True
        return False


    def on_pick_spm(self):
        spm_file_list = QtGui.QFileDialog.getOpenFileNames(self, "File a speed tree file", self.work_root, "Speed Tree File(*.spm)")[0]

        for spm_file in spm_file_list:
            if os.path.isfile(spm_file):
                self.w_publish_file.listWidget_speed_tree.addItem(spm_file.replace('\\', '/'))
